...
class AuthorReceptionPageLinks extends React.Component {
	...
	render() {
		...
		const prevPublication = pageIndex > -1 && pageIndex > 0 ?
			<Link className="btn btn-default" to={urls.authorReceptionIndex(authorReceptionPages[pageIndex - 1])} hash="#prevnext=1">◂ Previous</Link> : null;

		const nextPublication = pageIndex > -1 && pageIndex < authorReceptionPages.length - 1 ?
			<Link className="btn btn-default" to={urls.authorReceptionIndex(authorReceptionPages[pageIndex + 1])} hash="#prevnext=1">Next ▸</Link> : null;
